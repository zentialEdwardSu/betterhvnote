...
class AndroidPairingController(context: Context) : PairingController {
  ...
  override val pairedClients: List<PairedDevice>
    get() = database.readableDatabase.query(
      ...
      null,
      null,
      "last_used_at DESC, paired_at DESC",
    ).use { cursor ->
      buildList {
  ...
  override fun begin(localDeviceName: String): PairingOffer {
    ...
    return PairingOffer(
      ...
      nonce = TransferCrypto.randomBytes(16),
  ...
  override fun accept(offer: PairingOffer, remoteDeviceName: String): PairingConfirmation {
    ...
    return PairingConfirmation(
      local.deviceId,
      offer.deviceId,
      offer.deviceName.ifBlank { remoteDeviceName },
      TransferCrypto.verificationCode(key, transcript),
      key,
    )
  }
  ...
  override fun unpair(deviceId: String?) {
    ...
    if (deviceId == null) {
      ...
      writable.delete(TABLE, "device_id=?", arrayOf(deviceId))
    }
  }
  ...
  private fun replaceIdentity(old: PairedDevice, deviceId: String, deviceName: String): PairedDevice {
    ...
    writable.transaction {
      delete(TABLE, "device_id=?", arrayOf(old.id))
      insert(this, resolved, secret)
    }
    return resolved
  }
  ...
  private fun getOrCreateLocalDeviceId(): String = prefs.getString(KEY_LOCAL_ID, null)
    ?: UUID.randomUUID().toString().also { id ->
      prefs.edit { putString(KEY_LOCAL_ID, id) }
    }
