...
private class GattConnectionException(val status: Int, newState: Int) :
...
class BleGattSession private constructor(private val context: Context, private val device: BluetoothDevice) :
  AutoCloseable {
  ...
  private val callback = object : BluetoothGattCallback() {
    ...
    override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
      if (status != BluetoothGatt.GATT_SUCCESS) {
        ready.completeExceptionally(IllegalStateException("GATT discovery failed: $status"));
        return
      }
      ...
      val result = response ?: run {
        ready.completeExceptionally(IllegalStateException("BetterHv BLE response characteristic missing"));
        return
      }
    ...
    override fun onDescriptorWrite(gatt: BluetoothGatt, descriptor: BluetoothGattDescriptor, status: Int) {
      if (status == BluetoothGatt.GATT_SUCCESS) {
        ...
        ready.completeExceptionally(IllegalStateException("GATT subscribe failed: $status"))
      }
    }
    ...
    override fun onMtuChanged(gatt: BluetoothGatt, mtu: Int, status: Int) {
      if (status == BluetoothGatt.GATT_SUCCESS) {
        ...
        mtuReady.completeExceptionally(IllegalStateException("GATT MTU negotiation failed: $status"))
      }
    }
    ...
    override fun onCharacteristicWrite(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) {
      if (closed) return
      if (status == BluetoothGatt.GATT_SUCCESS) {
        writeWaiter?.complete(Unit)
      } else {
        writeWaiter?.completeExceptionally(IllegalStateException("GATT frame write failed: $status"))
      }
    }
    ...
    override fun onCharacteristicRead(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) {
      ...
      if (status == BluetoothGatt.GATT_SUCCESS) {
        ...
        identityWaiter?.completeExceptionally(IllegalStateException("GATT identity read failed: $status"))
      }
    }
  ...
  private suspend fun open() {
    gatt = if (Build.VERSION.SDK_INT >= NEW_GATT_CONNECT_API_LEVEL) {
  ...
  companion object {
    private const val NEW_GATT_CONNECT_API_LEVEL = 37
