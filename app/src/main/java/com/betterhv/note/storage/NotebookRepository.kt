...
class NotebookRepository(context: Context) : AutoCloseable {
    ...
    fun listNotebooks(currentId: UUID): List<NotebookSummary> {
        ...
        db.query(
            "notebooks", arrayOf("id", "title", "kind", "updated_at"),
    ...
    fun createBlankNotebook(
        ...
        try {
            ...
            insertBlankPage(db, pageId, notebookId, width, height, now, templateId)
            insertPageOrder(db, notebookId, pageId, 0)
            writeMetadata(db, ACTIVE_NOTEBOOK_KEY, notebookId.toString())
            writeMetadata(db, lastPageKey(notebookId), pageId.toString())
    ...
    fun createPdfNotebook(import: PdfImportCommit): UUID {
        ...
        db.beginTransaction()
        try {
            insertNotebook(db, notebookId, normalizedTitle, now, NotebookKind.PDF)
            import.pages.forEachIndexed { position, imported ->
                val pageId = UUID.randomUUID()
                if (firstPageId == null) firstPageId = pageId
                insertPdfPage(db, pageId, notebookId, imported, now)
                insertPageOrder(db, notebookId, pageId, position)
            }
            ContentValues().apply {
                ...
                put("created_at", now)
            }.also { db.insertOrThrow("pdf_documents", null, it) }
            writeMetadata(db, ACTIVE_NOTEBOOK_KEY, notebookId.toString())
            writeMetadata(db, lastPageKey(notebookId), requireNotNull(firstPageId).toString())
            insertJournal(db, notebookId, "create:pdf-notebook")
            db.setTransactionSuccessful()
        } finally {
            db.endTransaction()
        }
    ...
    fun pdfDocument(notebookId: UUID): PdfDocumentRecord? = store.readableDatabase.query(
        ...
        "notebook_id=?", arrayOf(notebookId.toString()), null, null, null, "1"
    ).use { cursor ->
        if (!cursor.moveToFirst()) return@use null
        PdfDocumentRecord(
            notebookId, cursor.getString(0), cursor.getString(1), cursor.getString(2),
            cursor.getLong(3), cursor.getString(4), cursor.getInt(5), cursor.getLong(6)
        )
    }
    ...
    fun createLinkedNotePage(sourcePageId: UUID, width: Float, height: Float): UUID {
        ...
        db.beginTransaction()
        try {
            insertLinkedNotePage(db, pageId, source.first, sourcePageId, width, height, now)
            replacePageOrder(db, source.first, order.toMutableList().apply { add(insertAt, pageId) })
            ContentValues().apply { put("updated_at", now) }.also { values ->
                db.update("notebooks", values, "id=?", arrayOf(source.first.toString()))
            }
            insertJournal(db, source.first, "create:linked-note")
            db.setTransactionSuccessful()
        } finally {
            db.endTransaction()
        }
    ...
    private fun insertPdfAnchor(db: SQLiteDatabase, anchor: PdfAnchor) {
        ...
        ContentValues().apply {
            put("id", anchor.id.toString()); put("source_page_id", anchor.sourcePageId.toString())
            put("note_page_id", anchor.notePageId.toString()); put("note_object_id", anchor.noteObjectId.toString())
            put("kind", anchor.kind.name); put("normalized_left", b.left); put("normalized_top", b.top)
            put("normalized_right", b.right); put("normalized_bottom", b.bottom)
            put("selected_text", anchor.selectedText); put("ordinal", anchor.ordinal); put("created_at", anchor.createdAt)
    ...
    fun pdfAnchors(sourcePageId: UUID): List<PdfAnchor> {
        ...
        store.readableDatabase.query(
            "pdf_anchors",
            arrayOf("id", "note_page_id", "note_object_id", "kind", "normalized_left", "normalized_top", "normalized_right", "normalized_bottom", "selected_text", "ordinal", "created_at"),
            "source_page_id=?", arrayOf(sourcePageId.toString()), null, null, "ordinal"
        ).use { c ->
            while (c.moveToNext()) result += PdfAnchor(
    ...
    fun pdfAnchorsForNotePage(notePageId: UUID): List<PdfAnchor> {
        ...
        store.readableDatabase.query(
            ...
            "note_page_id=?", arrayOf(notePageId.toString()), null, null, "ordinal"
        ).use { c ->
            while (c.moveToNext()) result += PdfAnchor(
    ...
    fun pdfAnchorForNoteObject(noteObjectId: UUID): PdfAnchor? = store.readableDatabase.query(
        ...
        "note_object_id=?", arrayOf(noteObjectId.toString()), null, null, null, "1"
    ).use { c ->
    ...
    fun deleteNotebook(notebookId: UUID): NotebookDeletionResult {
        ...
        try {
            ...
            db.delete("metadata", "key=?", arrayOf(lastPageKey(notebookId)))
    ...
    fun loadPage(id: UUID): Page? {
        ...
        val page = db.query(
            ...
            val pdfSource = if (kind == PageKind.PDF_SOURCE) PdfPageSource(
                ...
                pdfToPage = Transform2D(
                    cursor.getFloat(13), cursor.getFloat(14), cursor.getFloat(15),
                    cursor.getFloat(16), cursor.getFloat(17), cursor.getFloat(18)
                )
            ) else null
            Page(
                id = id,
                ...
                pdfSource = pdfSource,
                templateId = if (kind == PageKind.PDF_SOURCE) null else
    ...
    private fun persistInternal(
        ...
        try {
            ...
            anchor?.let {
                validatePdfAnchor(db, it)
                insertPdfAnchor(db, it)
            }
            ContentValues().apply {
    ...
    private fun syncPageObjects(db: SQLiteDatabase, snapshot: PageSnapshot) {
        ...
        db.query(
            "objects", arrayOf("id"), "page_id=?",
            arrayOf(snapshot.metadata.id.toString()), null, null, null
        ).use { cursor ->
            while (cursor.moveToNext()) {
    ...
    fun referencedPdfAssets(): Set<String> {
        val result = LinkedHashSet<String>()
        store.readableDatabase.query(
            "pdf_documents", arrayOf("asset_path"), null, null, null, null, null
        ).use { cursor -> while (cursor.moveToNext()) result += cursor.getString(0) }
        return result
    }
    ...
    private fun loadNotebookMetadata(db: SQLiteDatabase, notebookId: UUID): Notebook? {
        val notebook = db.query(
            "notebooks", arrayOf("id", "title", "kind", "created_at", "updated_at"),
        ...
        db.rawQuery(
            ...
            while (cursor.moveToNext()) {
                ...
                val source = if (kind == PageKind.PDF_SOURCE) PdfPageSource(
                    cursor.getInt(9),
                    Bounds(cursor.getFloat(10), cursor.getFloat(11), cursor.getFloat(12), cursor.getFloat(13)),
                    Transform2D(
                        cursor.getFloat(14), cursor.getFloat(15), cursor.getFloat(16),
                        cursor.getFloat(17), cursor.getFloat(18), cursor.getFloat(19)
                    )
                ) else null
                notebook.addPageMetadata(
                    Notebook.PageMetadata(
                        ...
                        if (kind == PageKind.PDF_SOURCE) null else
    ...
    private fun insertPdfPage(
        ...
        id: UUID,
        notebookId: UUID,
        imported: PdfImportedPage,
        now: Long
    ) {
        val source = imported.source
        ContentValues().apply {
            put("id", id.toString()); put("notebook_id", notebookId.toString())
            put("width", imported.width); put("height", imported.height)
            put("bookmarked", 0); put("content_revision", 0L); put("kind", PageKind.PDF_SOURCE.name)
            put("pdf_page_index", source.sourcePageIndex)
            put("pdf_bound_left", source.bounds.left); put("pdf_bound_top", source.bounds.top)
            put("pdf_bound_right", source.bounds.right); put("pdf_bound_bottom", source.bounds.bottom)
            put("pdf_a", source.pdfToPage.a); put("pdf_b", source.pdfToPage.b)
            put("pdf_c", source.pdfToPage.c); put("pdf_d", source.pdfToPage.d)
            put("pdf_tx", source.pdfToPage.tx); put("pdf_ty", source.pdfToPage.ty)
            putNull("template_id")
    ...
    private fun insertLinkedNotePage(
        ...
        ContentValues().apply {
            put("id", id.toString()); put("notebook_id", notebookId.toString())
            put("width", width); put("height", height); put("bookmarked", 0); put("content_revision", 0L)
            put("kind", PageKind.LINKED_NOTE.name); put("parent_pdf_page_id", sourcePageId.toString())
    ...
    private fun notebookKind(db: SQLiteDatabase, notebookId: UUID): NotebookKind? = db.query(
        "notebooks", arrayOf("kind"), "id=?", arrayOf(notebookId.toString()), null, null, null, "1"
    ...
    private fun pageOwnerAndKind(db: SQLiteDatabase, pageId: UUID): Pair<UUID, PageKind>? = db.query(
        "pages", arrayOf("notebook_id", "kind"), "id=?", arrayOf(pageId.toString()), null, null, null, "1"
    ).use { c ->
        if (!c.moveToFirst()) null else UUID.fromString(c.getString(0)) to
            enumValueOrDefault(c.getString(1), PageKind.BLANK)
    }

    private fun parentPdfPageId(db: SQLiteDatabase, pageId: UUID): UUID? = db.query(
        "pages", arrayOf("parent_pdf_page_id"), "id=?", arrayOf(pageId.toString()), null, null, null, "1"
    ).use { c ->
    ...
    private fun objectBelongsToPage(db: SQLiteDatabase, objectId: UUID, pageId: UUID): Boolean = db.query(
        "objects", arrayOf("id"), "id=? AND page_id=?",
        arrayOf(objectId.toString(), pageId.toString()), null, null, null, "1"
    ).use { it.moveToFirst() }

    private fun validatePdfAnchor(db: SQLiteDatabase, anchor: PdfAnchor) {
        val b = anchor.normalizedBounds
        require(b.left in 0f..1f && b.top in 0f..1f && b.right in 0f..1f && b.bottom in 0f..1f &&
            b.right > b.left && b.bottom > b.top) { "Anchor bounds must be normalized and non-empty" }
    ...
    private fun syncPagesAndOrder(db: SQLiteDatabase, change: DocumentChange) {
        ...
        change.pages.forEach { page ->
            val values = ContentValues().apply {
                ...
                if (page.parentPdfPageId == null) putNull("parent_pdf_page_id")
                else put("parent_pdf_page_id", page.parentPdfPageId.toString())
                val source = page.pdfSource
                if (source == null) {
                    putNull("pdf_page_index"); putNull("pdf_bound_left"); putNull("pdf_bound_top")
                    putNull("pdf_bound_right"); putNull("pdf_bound_bottom")
                    putNull("pdf_a"); putNull("pdf_b"); putNull("pdf_c"); putNull("pdf_d")
                    putNull("pdf_tx"); putNull("pdf_ty")
                } else {
                    put("pdf_page_index", source.sourcePageIndex)
                    put("pdf_bound_left", source.bounds.left); put("pdf_bound_top", source.bounds.top)
                    put("pdf_bound_right", source.bounds.right); put("pdf_bound_bottom", source.bounds.bottom)
    ...
    private fun upsertObject(db: SQLiteDatabase, pageId: UUID, obj: PageObject) {
        ...
        when (obj) {
            ...
            is ImageObject -> updateOrInsert(db, "images", ContentValues().apply {
            ...
            is TextObject -> updateOrInsert(db, "texts", ContentValues().apply {
