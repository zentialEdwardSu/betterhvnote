...
data class ViewportTransform(
    ...
    fun screenToPage(x: Float, y: Float): FloatArray =
        floatArrayOf((x - offsetX) / scale, (y - offsetY) / scale)

    fun pageToScreen(x: Float, y: Float): FloatArray =
        floatArrayOf(x * scale + offsetX, y * scale + offsetY)
...
internal object PdfViewportMath {
    ...
    fun clamp(
        ...
        else (centerX + state.panX).coerceIn(viewWidth - contentWidth, 0f)
        ...
        else (centerY + state.panY).coerceIn(viewHeight - contentHeight, 0f)
    ...
    fun zoomAt(
        ...
        return clamp(
            ...
            pageWidth, pageHeight, viewWidth, viewHeight
        )
    }
