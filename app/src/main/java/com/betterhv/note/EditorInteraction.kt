...
enum class ShortcutAction(val storageId: String, val label: String, val scene: ShortcutScene) {
...
data class HardwareShortcutBindings(val values: Map<ShortcutScene, Map<HardwareKeyId, ShortcutAction>> = emptyMap()) {
...
class PageManagerShortcutContext(private val performer: (ShortcutAction) -> Boolean) :
  FlyoutHardwareShortcutContext(ShortcutScene.PAGE_MANAGER) {
  override fun perform(action: ShortcutAction): Boolean = action.scene == scene && performer(action)
}

class InsertShortcutContext(private val performer: (ShortcutAction) -> Boolean) :
