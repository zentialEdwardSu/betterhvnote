...
object HitTest {
    ...
    fun pointSegmentDistanceSquared(
        px: Float, py: Float,
        ax: Float, ay: Float,
        bx: Float, by: Float
    ): Float {
    ...
    fun strokeTouchesLocal(
        points: List<com.betterhv.note.ink.InkPoint>,
        lx: Float, ly: Float,
        radius: Float
    ): Boolean {
    ...
    ): Float = (bx - ax) * (py - ay) - (by - ay) * (px - ax)

    private fun pointOnSegment(
        px: Float, py: Float, ax: Float, ay: Float, bx: Float, by: Float
    ): Boolean = px >= minOf(ax, bx) - EPSILON && px <= maxOf(ax, bx) + EPSILON &&
