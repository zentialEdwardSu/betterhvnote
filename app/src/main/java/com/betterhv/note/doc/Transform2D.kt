 ...
 */
data class Transform2D(val a: Float, val b: Float, val c: Float, val d: Float, val tx: Float, val ty: Float) {
  ...
  fun mapPoint(x: Float, y: Float): FloatArray = floatArrayOf(a * x + c * y + tx, b * x + d * y + ty)
  ...
  companion object {
    ...
    fun translate(dx: Float, dy: Float): Transform2D = Transform2D(a = 1f, b = 0f, c = 0f, d = 1f, tx = dx, ty = dy)

    /** Uniform scale by [scale] about [centerX], [centerY] (spec §39). */
    fun scaleAbout(centerX: Float, centerY: Float, scale: Float): Transform2D = Transform2D(
      a = scale,
      b = 0f,
      c = 0f,
      d = scale,
