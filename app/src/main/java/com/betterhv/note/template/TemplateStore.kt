...
class TemplateStore private constructor(private val context: Context) : TemplateResolver {
    ...
    @Synchronized
    override fun resolve(templateId: String?): TemplateDefinition? =
        catalog.find(templateId ?: DEFAULT_TEMPLATE_ID)
    ...
    override fun visualFingerprint(templateId: String?): String {
        ...
        return if (resolved != null) resolved.fingerprint
        else "missing:$requested:${catalog.find(DEFAULT_TEMPLATE_ID)?.fingerprint.orEmpty()}"
    }
        ...
        return parsed.entries.mapNotNull { entry ->
            ...
            if (cached != null) {
                runCatching { validateAsset(cached, entry) }.onFailure {
                    ...
                    TemplateDefinition(
                        entry, cached, sha256(cached), TemplateAvailability.CACHED,
                        "外部来源不可用，正在使用最后可用版本"
                    )
                }
            } else {
                TemplateDefinition(
                    entry, null, "unavailable:${entry.id}", TemplateAvailability.INVALID,
                    "模板文件缺失或损坏"
                )
            }
        }
    ...
    private fun seedBuiltins(uri: Uri) {
        ...
        val targetJson = if (manifestDoc == null) builtinJson else {
