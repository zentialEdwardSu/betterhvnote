...
data class PenColorChoice(val id: String, val label: String, val argb: Int)
...
data class PenPreset(val colorIndex: Int = DEFAULT_COLOR_INDEX, val widthLevel: Int = DEFAULT_WIDTH_LEVEL) {
  companion object {
...
data class PenToolbarSettings(
  ...
  fun selectSlot(slot: Int): PenToolbarSettings = copy(activeSlot = slot.takeIf { it in 0 until SLOT_COUNT } ?: 0)
...
object PenSettingsCodec {
  data class Encoded(val activeTypeId: String, val colors: Map<String, Int>, val widths: Map<String, Int>)
  ...
  fun decode(activeTypeId: String?, colors: Map<String, Int>, widths: Map<String, Int>): PenSettings {
