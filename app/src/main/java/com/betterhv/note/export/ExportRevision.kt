...
object ExportRevision {
  fun fingerprint(format: ExportFormat, sources: List<ExportPageSource>, rendererVersion: Int): String {
  ...
  fun stalePageCount(sources: List<ExportPageSource>, cachedRevisions: Map<UUID, Long>): Int =
    sources.count { cachedRevisions[it.id] != it.contentRevision }
}
