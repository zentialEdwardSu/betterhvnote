...
class ExportTaskRepository(context: Context) : AutoCloseable {
    ...
    fun listSummaries(): List<ExportTaskSummary> = listTasksRaw().map { task ->
        ...
        val state = ExportRevision.state(
            missing, fileExists, record?.fingerprint, currentFingerprint, record?.lastError != null
        )
    ...
    fun listNotebookOptions(): List<ExportNotebookOption> {
        ...
        return db.query(
            "notebooks", arrayOf("id", "title"), null, null, null, null, "updated_at DESC"
        ).use { cursor ->
            buildList {
    ...
    fun cachedPage(taskId: UUID, source: ExportPageSource): File? = store.readableDatabase.query(
        "export_page_cache", arrayOf("content_revision", "cache_path"),
        "task_id=? AND page_id=?", arrayOf(taskId.toString(), source.id.toString()),
        null, null, null
    ).use { cursor ->
    ...
    fun recordCachedPage(taskId: UUID, source: ExportPageSource, file: File): File? {
        val previous = store.readableDatabase.query(
            "export_page_cache", arrayOf("cache_path"), "task_id=? AND page_id=?",
            arrayOf(taskId.toString(), source.id.toString()), null, null, null
        ...
        ContentValues().apply {
            ...
            store.writableDatabase.insertWithOnConflict(
                "export_page_cache", null, it, SQLiteDatabase.CONFLICT_REPLACE
            )
        }
    ...
    fun removeObsoleteCaches(taskId: UUID, retained: Set<UUID>): List<File> {
        ...
        val db = store.writableDatabase
        db.query(
            "export_page_cache", arrayOf("page_id", "cache_path"), "task_id=?",
        ...
        if (retained.isEmpty()) {
            ...
            db.delete(
                "export_page_cache", "task_id=? AND page_id NOT IN ($placeholders)", args.toTypedArray()
            )
        }
        return removed
    }
    ...
    fun deleteTask(id: UUID): List<File> {
        ...
        store.readableDatabase.query(
            "export_page_cache", arrayOf("cache_path"), "task_id=?",
    ...
    fun referencedInternalFiles(): Set<String> {
        ...
        store.readableDatabase.query(
            "export_tasks", arrayOf("artifact_path"), "artifact_path IS NOT NULL",
            null, null, null, null
        ).use { cursor -> while (cursor.moveToNext()) paths += File(cursor.getString(0)).absolutePath }
        store.readableDatabase.query(
            "export_page_cache", arrayOf("cache_path"), null, null, null, null, null
        ).use { cursor -> while (cursor.moveToNext()) paths += File(cursor.getString(0)).absolutePath }
        return paths
    }

    fun fingerprint(task: ExportTask, sources: List<ExportPageSource>): String {
        return ExportRevision.fingerprint(task.format, sources, RENDERER_VERSION)
    }

    fun displayName(task: ExportTask): String {
        return ExportFileNames.displayName(notebookTitle(task.notebookId), task.id, task.format)
    }
    ...
    private fun listTasksRaw(): List<ExportTask> {
        ...
        db.query(
            "export_task_pages", arrayOf("task_id", "page_id", "position"),
            null, null, null, null, "task_id, position"
        ).use { cursor ->
            while (cursor.moveToNext()) {
        ...
        return db.query(
            "export_tasks", arrayOf("id", "notebook_id", "scope", "format", "created_at", "last_used_at"),
            null, null, null, null, "last_used_at DESC, created_at DESC"
        ).use { cursor ->
            buildList {
                while (cursor.moveToNext()) {
                    val id = UUID.fromString(cursor.getString(0))
                    add(ExportTask(
                        id, UUID.fromString(cursor.getString(1)), ExportScope.valueOf(cursor.getString(2)),
    ...
    private fun orderedExistingPages(notebookId: UUID): List<UUID> = store.readableDatabase.query(
        "page_order", arrayOf("page_id"), "notebook_id=?", arrayOf(notebookId.toString()),
        null, null, "position"
    ...
    private fun stalePageCount(task: ExportTask, sources: List<ExportPageSource>): Int =
        if (task.format == ExportFormat.PNG) {
            val record = artifactRecord(task.id)
            if (record?.fingerprint == fingerprint(task, sources) && record.path?.let(::File)?.isFile == true) 0
            else sources.size
        } else sources.count { cachedPage(task.id, it) == null }

    private fun artifactRecord(taskId: UUID): ArtifactRecord? = store.readableDatabase.query(
        "export_tasks",
        arrayOf("artifact_id", "artifact_path", "artifact_fingerprint", "artifact_sha256", "artifact_created_at", "last_error"),
        "id=?", arrayOf(taskId.toString()), null, null, null
    ).use { cursor ->
        if (!cursor.moveToFirst()) null else ArtifactRecord(
            cursor.getString(0)?.let(UUID::fromString), cursor.getString(1), cursor.getString(2),
            cursor.getBlob(3), if (cursor.isNull(4)) null else cursor.getLong(4), cursor.getString(5)
        )
    }
    ...
    private fun notebookExists(id: UUID): Boolean = store.readableDatabase.query(
        "notebooks", arrayOf("id"), "id=?", arrayOf(id.toString()), null, null, null
    ).use { it.moveToFirst() }

    private fun notebookTitle(id: UUID): String? = store.readableDatabase.query(
        "notebooks", arrayOf("title"), "id=?", arrayOf(id.toString()), null, null, null
    ).use { if (it.moveToFirst()) it.getString(0) else null }

    override fun close() = store.close()

    private data class ArtifactRecord(
        val id: UUID?, val path: String?, val fingerprint: String?, val sha256: ByteArray?,
