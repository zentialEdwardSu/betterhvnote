...
object StrokeRenderer {
  ...
  private fun density(): Int = if (Build.VERSION.SDK_INT == 0x22) HWPenEngine.DENSITY_3576 else HWPenEngine.DENSITY_3566
  ...
  private fun commitStrokeEraserPixel(bmp: Bitmap, raw: FloatArray): Boolean {
    ...
    while (i < n) {
      ...
      while (ey <= y1) {
        var ex = x0
        while (ex <= x1) {
          bmp[ex, ey] = android.graphics.Color.TRANSPARENT
          ex++
        }
        ey++
      }
