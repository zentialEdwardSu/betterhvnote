...
class QueueCoordinator(private val store: QueueStore, private val clock: () -> Long = System::currentTimeMillis) {
...
class QueueTransferLease(override val offer: TransferOffer, private val coordinator: QueueCoordinator) : TransferLease {
  private var finished = false

  override fun heartbeat() {
    if (!finished) coordinator.heartbeat(offer.item.id)
  }
  override fun markTransferring() {
    if (!finished) coordinator.transition(offer.item.id, QueueState.TRANSFERRING)
  }
  override fun markAwaitingCommit() {
    if (!finished) coordinator.transition(offer.item.id, QueueState.AWAITING_COMMIT)
  }
  override fun commit() {
    if (!finished) {
