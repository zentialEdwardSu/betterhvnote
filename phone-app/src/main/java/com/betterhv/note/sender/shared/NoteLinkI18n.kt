...
enum class NoteLinkLanguage(val storageId: String) {
  ...
  companion object {
    fun fromStorageId(value: String?): NoteLinkLanguage = entries.firstOrNull { it.storageId == value } ?: SYSTEM
  }
}
...
fun NoteLinkLanguage.text(chinese: String, english: String): String = if (isChinese()) chinese else english
...
fun noteLinkText(chinese: String, english: String): String = NoteLinkI18n.language.text(chinese, english)
