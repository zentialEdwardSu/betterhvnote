...
class DesktopQueueRepository(private val paths: DesktopPaths) : QueueStore, AutoCloseable {
    ...
    @Synchronized fun enqueueFiles(files: List<File>, destinationDeviceId: String?): List<QueueItem> = files.map { source ->
        ...
        require(source.length() <= TransferLimits.MAX_IMAGE_BYTES) { "图片不能超过 64 MiB" }
        val mime = Files.probeContentType(source.toPath())?.takeIf { it.startsWith("image/") }
            ?: when (source.extension.lowercase()) { "png" -> "image/png"; "jpg", "jpeg" -> "image/jpeg"; else -> error("只支持图片") }
    ...
    @Synchronized fun counts(): Pair<Int, Int> = items().filter { it.state == QueueState.PENDING }.let { available ->
        available.count { it.kind == ContentKind.IMAGE } to available.count { it.kind == ContentKind.TEXT }
    }
