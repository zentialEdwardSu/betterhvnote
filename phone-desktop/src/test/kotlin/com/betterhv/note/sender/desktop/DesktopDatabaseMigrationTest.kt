...
class DesktopDatabaseMigrationTest {
    ...
    private fun createQueueV1(file: File, id: UUID, hash: ByteArray) {
        DriverManager.getConnection("jdbc:sqlite:${file.absolutePath}").use { connection ->
            connection.createStatement().use { it.execute("""CREATE TABLE queue_items(
            ...
            connection.prepareStatement("INSERT INTO queue_items VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)").use {
                it.setString(1, id.toString()); it.setString(2, null); it.setString(3, "IMAGE")
                it.setString(4, "image/png"); it.setLong(5, 42); it.setBytes(6, hash); it.setLong(7, 10)
                it.setLong(8, 0); it.setString(9, "PENDING"); it.setString(10, "legacy.png")
                it.setObject(11, null); it.setObject(12, null); it.setString(13, "outbox/$id.bin"); it.setObject(14, null)
                it.executeUpdate()
            }
        }
    }
    ...
    private fun createInboxV1(file: File, id: UUID, hash: ByteArray, payload: File) {
        DriverManager.getConnection("jdbc:sqlite:${file.absolutePath}").use { connection ->
            connection.createStatement().use { it.execute("""CREATE TABLE inbox_exports(
            ...
            connection.prepareStatement("INSERT INTO inbox_exports VALUES(?,?,?,?,?,?,?,?,?,?,?)").use {
                it.setString(1, id.toString()); it.setString(2, "note"); it.setString(3, "legacy.pdf")
                it.setString(4, "application/pdf"); it.setLong(5, 42); it.setBytes(6, hash); it.setString(7, payload.absolutePath)
                it.setString(8, "COMPLETE"); it.setLong(9, 10); it.setLong(10, 11); it.setObject(11, null)
                it.executeUpdate()
            }
        }
    }
